...
c
c       input:
c       lproj -- maximum usable length (in double complex elements)
c                of the array proj
...
c               proj must be at least m+1 + 2*n*(krank+1) double complex
...
c
     ...
     1          lra,ier,k
        ...
        double complex p1,p2,p3,p4,proj(*)
        external matveca
c
c
        ier = 0
...
c
c       input:
c       lra -- maximum usable length (in double complex elements)
c              of array ra
...
c             ra must be at least 2*n*krank double complex elements long
c       ier -- 0 when the routine terminates successfully;
c              -1000 when lra is too small
c
c       work:
c       w -- must be at least m+2*n+1 double complex elements long
c
c       _N.B._: ra must be at least 2*n*krank double complex elements long.
c               Also, the algorithm used by this routine is randomized.
c
        implicit none
        integer m,n,lw,krank,ix,lx,iy,ly,iscal,lscal,lra,ier
        double precision eps
        double complex p1,p2,p3,p4,ra(n,*),w(m+2*n+1)
        external matveca
c
c
        lw = 0
...
c
        ...
        double precision eps,enorm
        double complex x(m),ra(n,2,*),p1,p2,p3,p4,scal(n+1),y(n)
