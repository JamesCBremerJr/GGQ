...
c               proj must be at least n*(2*n2+1)+n2+1 double precision elements
...
c
        ...
        double precision eps,a(m,n),proj(*),work(17*m+70)
c
c
...
cc
       ...
       subroutine iddp_aid_mt(eps,m,n,a,work,krank,list,proj)
c
c       computes the ID of the matrix a, i.e., lists in list
c       the indices of krank columns of a such that
c
...
c       Find the rank of a.
c
        call idd_estrank_mt(eps,m,n,a,work,kranki,proj)
c
c
        if(kranki .eq. 0) call iddp_aid0(eps,m,n,a,krank,list,proj,
     1                                   proj(m*n+1))
...
c               proj doubles as a work array in the present routine, so
c               must be at least m*n double precision elements long
c
c       work:
c       rnorms -- must be at least n double precision elements long
c
c       _N.B._: proj must be at least m*n double precision elements long
c
        implicit none
        integer m,n,krank,list(n),j,k
        double precision eps,a(m,n),proj(m,n),rnorms(n)
c
c
c       Copy a into proj.
c
...
c
        ...
        double precision eps,proj(n2*n),rnorms(n)
c
c
...
c             must be at least n*n2+(n+1)*(n2+1) double precision elements long
c
c       _N.B._: ra must be at least n*n2+(n2+1)*(n+1) double precision
...
c
        ...
        double precision eps,a(m,n),ra(*),w(17*m+70)
c
c
...
c
c
        subroutine idd_estrank_mt(eps,m,n,a,w,krank,ra)
c
c       estimates the numerical rank krank of an m x n matrix a
c       to precision eps. This routine applies n2 random vectors
c       to a, obtaining ra, where n2 is the greatest integer
...
c
        ...
        lra = lra+lscal
c
        call idd_estrank0_mt(eps,m,n,a,w,n2,krank,ra(ira),ra(irat),
     1                    ra(iscal))
c
c
        return
...
c
        ...
        double precision a(m,n),ra(n2,n),scal(n2+1),eps,residual,
...
c
        ...
        subroutine idd_estrank0_mt(eps,m,n,a,w,n2,krank,ra,rat,scal)
c
c       routine idd_estrank serves as a memory wrapper
c       for the present routine. (Please see routine idd_estrank
c       for further documentation.)
c
     ...
     1         w(17*m+70),rat(n,n2+1),ss,ssmax
c
        double precision, allocatable :: wcopy(:)
c
c
c       Apply the random matrix to every column of a, obtaining ra.
c
...
!$OMP DO
        do k = 1,n
          call idd_frm(m,n2,wcopy,a(1,k),ra(1,k))
        enddo ! k
!$OMP END DO
!$OMP END PARALLEL
c
