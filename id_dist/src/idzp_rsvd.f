...
c
c       input:
c       lw -- maximum usable length (in double complex elements)
c             of the array w
...
c             as double complex numbers whose imaginary parts are zeros
c       w -- array containing the singular values and singular vectors
c            of a; w doubles as a work array, and so must be at least
c            (krank+1)*(3*m+5*n+11)+8*krank^2 double complex elements long,
...
c               double complex elements long, where krank is the rank
...
c
     ...
     1          iwork,lwork,ier,lproj,iu,iv,is,lu,lv,ls,iui,ivi,isi,k
        ...
        double complex p1t,p2t,p3t,p4t,p1,p2,p3,p4,w(*)
...
c
        ...
        double precision s(krank)
        double complex p1t,p2t,p3t,p4t,p1,p2,p3,p4,u(m,krank),
     1             v(n,krank),proj(krank,n-krank),col(m*krank),
...
c
c       copies the double precision array a into the double complex array b.
c
c       input:
c       n -- length of a and b
c       a -- double precision array to be copied into b
c
c       output:
c       b -- double complex copy of a
c
        integer n,k
